...
**Текущий этап:** CLI + **FastAPI** + **Telegram-бот** + **Docker Compose** (Forum Topics, relay, SQLite-логи).
...
- **Docker** — два контейнера (`api` + `bot`), общий volume Chroma/SQLite — см. [`DOCKER.md`](DOCKER.md)
...
| `Dockerfile`, `docker-compose.yml` | Контейнеризация API + бота |
...
### Docker (рекомендуется для VPS / демо)
...
| Сервис | Назначение |
|--------|------------|
| `api` | FastAPI `:8000`, индексирует Chroma при первом старте |
| `bot` | Telegram polling, стартует после healthcheck API |

Подробно: команды, volume, переиндексация, troubleshooting — **[`DOCKER.md`](DOCKER.md)**.
...
| `API_HOST` / `API_PORT` | `127.0.0.1` / `8000` | FastAPI (`0.0.0.0` в Docker — см. `DOCKER.md`) |
...
- [x] Docker Compose (API + Telegram-бот)
- [ ] Веб-чат (HTML/JS → `/ask`)
